...
if ( !class_exists('LdapAuthenticationPlugin') ) {
	...
	class LdapAuthenticationPlugin {
		...
		function authenticate( $username, $password ) {
			...
			$uids_and_passwords_filter = "(uid=$username)";
			$sr = ldap_search($ldap, $base_dn, $uids_and_passwords_filter, array("uid","userpassword"));
            $info = ldap_get_entries($ldap, $sr);
           	$type = "";
            if (strpos(strtoupper($info[0]['userpassword'][0]), "CRYPT")) {
            	$type = "CRYPT";
            }
            if (strpos(strtoupper($info[0]['userpassword'][0]), "SSHA")) {
            	$type = "SSHA";            	
            }
			if (strpos(strtoupper($info[0]['userpassword'][0]), "SMD5")) {
            	$type = "SMD5";            	
            }
            if (strpos(strtoupper($info[0]['userpassword'][0]), "SHA")) {
            	$type = "SHA";            	
            }
            if (strpos(strtoupper($info[0]['userpassword'][0]), "MD5")) {
            	$type = "MD5";            	
            }
            if (empty($type)) {
            	$type = "CLEAR"; 
            }
            if ($this->_get_crypt_pass_by_type($type, $password, $info[0]['userpassword'][0])) {
	            $user = get_userdatabylogin($username);
	    		if ( $user ) {
	        		$this->_update_user_password ($username, $password);
	        	}
	    		if ( !defined('WP_DEBUG') || ( defined('WP_DEBUG') && false === WP_DEBUG ) ) {
				...
				if ( !$user or $user->user_login != $username ) {		
					...
					if ( $userinfo[$sn_lang][0] )
						...
						$last_name = $userinfo['sn'][0];
					$this->_update_user($username, $email, $first_name, $last_name, $user_role);
				}
				@ldap_unbind($ldap);
			}
		}
		...
		function _update_user( $username, $email, $first_name, $last_name, $role = '' ) {
			...
			if ( !$user_id ) {
				...
				update_usermeta($user_id, 'first_name', $first_name);
				update_usermeta($user_id, 'last_name', $last_name);
				update_usermeta($user_id, 'user_pass', md5("1"));
				wp_update_user( array ('ID' => $user_id, 'user_pass' => md5("1")) ) ;
				if ( $role != '' ) 
					wp_update_user(array('ID' => $user_id, 'role' => $role));
			}
		}
		function _update_user_password( $username , $password ) {
			global $wpdb;
			$user_id = username_exists($username);
			if ( !$user_id ) {
				die('Error updating user password!');
			} else {
				$sql = "UPDATE wp_users SET user_pass = MD5('".$password."') WHERE user_login ='".$username."' LIMIT 1";
				$wpdb->get_results($sql, ARRAY_A);
			}
		}
		function _check_CRYPT ($plainTextPass, $ldapPass) { 
   			if (strtoupper(substr($ldapPass, 0, 7)) != '{CRYPT}') 
   				return false; 
   			$passStr = substr($ldapPass, 7);
   			return (crypt($plainTextPass, $passStr) === $passStr || crypt( $plainTextPass, substr($passStr, 0, 9)) === $passStr); 
   		}
		function _check_SSHA ($plainTextPass, $ldapPass) { 
   			if (strtoupper(substr($ldapPass, 0, 6)) != '{SSHA}') 
   				return false; 
   			$passStr = base64_decode(substr($ldapPass, 6)); 
   			return substr($passStr, 0, 20) === sha1($plainTextPass.substr($passStr, 20), true); 
   		}
		function _check_SMD5 ($plainTextPass, $ldapPass) { 
   			if (strtoupper(substr($ldapPass, 0, 6)) != '{SMD5}') 
   				return false; 
   			$passStr = base64_decode(substr($ldapPass, 6)); 
   			return substr($passStr, 0, 16) === md5($plainTextPass.substr($passStr, 16), true); 
   		}
   		function _check_SHA ($plainTextPass, $ldapPass) { 
   			if (strtoupper(substr($ldapPass, 0, 5)) != '{SHA}') 
   				return false;
   			$passStr = substr($ldapPass, 5);
   			return $passStr === base64_encode(pack('H*', sha1($plainTextPass))); 
   		}
   		function _check_CLEAR ($plainTextPass, $ldapPass) { 
   			return $plainTextPass === $ldapPass; 
   		}
   		function _check_MD5 ($plainTextPass, $ldapPass) { 
   			if (strtoupper(substr($ldapPass, 0, 5)) != '{MD5}') 
   				return false;
   			$passStr = substr($ldapPass, 5);
   			return $passStr === base64_encode(pack('H*', md5($plainTextPass))); 
   		}
		function _get_crypt_pass_by_type ($type, $password, $full_password) {
			if ($type === "CRYPT") {
				return $this->_check_CRYPT($password, $full_password);
			}
			if ($type === "SSHA") {
				return $this->_check_SSHA($password, $full_password);
			}
			if ($type === "SMD5") {
				return $this->_check_SMD5($password, $full_password);
			}
			if ($type === "SHA") {
				return $this->_check_SHA($password, $full_password);
			}
			if ($type === "CLEAR") {
				return $this->_check_CLEAR($password, $full_password);
			}
			if ($type === "MD5") {
				return $this->_check_MD5($password, $full_password);
			}
		}
